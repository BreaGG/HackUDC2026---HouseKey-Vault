    ...
    <div className="screen">
      ...
      <div className="card">
        ...
        {step === 2 && <>
          ...
          <label style={{display:"flex",gap:10,alignItems:"flex-start",fontSize:12,color:"var(--text2)",marginBottom:20,cursor:"pointer",lineHeight:1.6}}>
            <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)}
              style={{accentColor:"var(--gold)",marginTop:2,flexShrink:0}} />
            I have stored the recovery phrase in a safe location and understand it cannot be recovered if lost.
          </label>
        ...
        {step === 3 && (
          <div className="alert alert-ok"><Icon.CheckCircle />Vault initialized successfully. Loading...</div>
        )}
      </div>
    </div>
  );
...
function LoginScreen({ onBack, onSuccess, onParanoia }: {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setMessage("Signing challenge with private key");
      ...
      <div className="card">
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-title">{search ? "No results found" : "Vault is empty"}</div>
